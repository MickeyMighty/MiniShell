...
| `signal` | <signal.h> |  typedef void (*sighandler_t)(int); sighandler_t signal(int signum, sighandler_t handler); | ![diagram1](https://indradhanush.github.io/images/shell-part-3/signals.jpg) [check the [man](https://man7.org/linux/man-pages/man2/signal.2.html)]|
| `kill` | <sys/types.h> <signal.h> |  int kill(pid_t pid, int sig); | Is an event which is generated to notify a process or thread that some important situation has arrived. When a process or thread has received a signal, the process or thread will stop what its doing and take some action. Signal may be useful for inter-process communication. [check the [man](https://man7.org/linux/man-pages/man2/kill.2.html)]|
| `exit` | <unistd.h> | void _exit(int status); | [check the [man](https://man7.org/linux/man-pages/man2/exit.2.html) and check the [source](https://indradhanush.github.io/blog/writing-a-unix-shell-part-3/)]|
